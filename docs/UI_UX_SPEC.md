...
## Phase 15 Building Network UI

Era 4 preserves the world-first phone-board hierarchy. The Building cutaway remains visible behind contextual Grid/Build/Upgrade interaction, WATT's scratched service-console core remains accessible, and the first request uses `Authorize Connection` plus a concrete operator-payoff line. Building infrastructure cards load their authored SVG silhouettes through the reusable semantic glyph component; repeated drawer refreshes reconfigure compatible cards in place rather than accumulating replacement nodes.

Maintenance uses the shared blocking, vertically scrollable modal and presents Repair, Replace, Overclock, exact costs/effects, the choose-once rule, and a non-punitive guarantee. At 320 × 568 and 130% text, the status band compacts and the vital grid reflows so the modal remains horizontally contained and every option is reachable by scrolling. Predictive Reserve Guard exposes OFF/ARMED/ACTIVE text and target percentage in addition to color. The capstone report adds takeover statistics and civilian consequence copy, then triggers the established pullback sequence and stops on a disabled `Neighborhood Locked` action.
