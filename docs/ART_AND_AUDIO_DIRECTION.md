...
## Phase 15 Building Network production-functional layer

The project-original Building cutaway advances the approved visual escalation with dim ordinary floors, warm surviving windows, a dominant cyan vertical riser, service transformer/battery masses, rooftop solar/cooling, an amber emergency stairwell cable, a tiny bicycle remnant, and a distributed lobby WATT face. The same scratched physical core remains in the service console. Four authored representative paths and clustered ownership states avoid wire placement or one-object-per-count rendering. Brownout replaces route color with labeled isolated paths while WATT's eyes remain cyan; the capstone reveal frames a locked Neighborhood silhouette.

Seven project-original SVG infrastructure silhouettes and a procedural maintenance-decision cue extend the existing reusable asset/audio systems. No generated concept raster is integrated. This is production-functional code-native presentation and remains subject to the later gold-standard tactile painted replacement tracked by `ISSUE-007`; it does not promote flat Phase 12 scaffolding or claim final-art approval.
