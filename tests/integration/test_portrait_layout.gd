...
func _run() -> void:
	...
	for test_size in _portrait_sizes:
		...
		_check("Content v0.3.0" in content_version_label.text and "schema 1" in content_version_label.text, "%s content version label was '%s'" % [test_size, content_version_label.text])
		_check("2 eras" in content_counts_label.text and "1 infrastructure" in content_counts_label.text and "1 upgrade" in content_counts_label.text and "4 request" in content_counts_label.text, "%s content counts label was '%s'" % [test_size, content_counts_label.text])
